...
namespace dooqu_server
{
	namespace service
	{
		...
		public:
			...
			boost::function<void(const boost::system::error_code&, const string&)> callback_func_;


			http_request(boost::asio::io_service& io_service,
				const std::string& server, const std::string& path, boost::function<void(const boost::system::error_code&, const string&)> callback_func)
		...
		private:
				...
				tcp::resolver::iterator endpoint_iterator)
			{
				...
				else
				{
					...
					this->callback_func_(err, std::string());
					//delete this;
				}
			}
			...
			void handle_read_status_line(const boost::system::error_code& err)
			{
				if (!err)
				{
					...
					if (!response_stream || http_version.substr(0, 5) != "HTTP/")
					{
						...
						this->callback_func_(boost::asio::error::already_connected, std::string());
						//delete this;
						return;
					}
					if (status_code != 200)
