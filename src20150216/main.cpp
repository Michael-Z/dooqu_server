...
int main(int argc, char* argv[])
{
...

	{
		...
		for (int i = 0, j = 0, zone_id = 0; i < 55; i++)
		{
			...
			printf("create game {%s}, at game_zone:{%s}\n", curr_game_id, curr_zone_id);
		...
		while (std::strcmp(read_line.c_str(), "exit") != 0)
		{
			...
			else if (std::strcmp(read_line.c_str(), "test") == 0)
			{
	...
	printf("server were stoped,press any key to exit.\n");
	getchar();
