...
namespace dooqu_server
{
	namespace ddz
	{
		void log(char* content)
		{
			...
			fs.open("e:\\log.txt", std::ios::out | std::ios::app,  _SH_DENYWR);
		...
		ddz_plugin::ddz_plugin(game_service* service, char* game_id, char* title, double frequence, int game_argc, char* game_argv[]) : game_plugin(service, game_id, title, frequence)//, desk_count_(desk_count), multiple_(multiple), /*timer_(service->get_io_service()),*/ max_waiting_duration_(21000)
		{
			this->desk_count_ = std::atoi(game_argv[0]);
			this->multiple_ = std::atoi(game_argv[1]);
			this->max_waiting_duration_ = std::atoi(game_argv[2]);
		}

		ddz_plugin::~ddz_plugin()
		{
			//回收游戏桌子资源
			for(int i = 0; i < this->desk_count_; ++i)
			{
				ddz_desk* desk = this->desks_[i];
				if(desk != NULL)
					delete desk;
			}
		}
		...
		{
			this->regist("IDK", make_handler(ddz_plugin::on_join_desk_handle));
			this->regist("RDY", make_handler(ddz_plugin::on_client_ready_handle));
			this->regist("BID", make_handler(ddz_plugin::on_client_bid_handle));
			this->regist("CAD", make_handler(ddz_plugin::on_client_card_handle));
			this->regist("MSG", make_handler(ddz_plugin::on_client_msg_handle));
			this->regist("PNG", make_handler(ddz_plugin::on_client_ping_handle));
			this->regist("RBT", make_handler(ddz_plugin::on_client_robot_handle));
			this->regist("BYE", make_handler(ddz_plugin::on_client_bye_handle));

			//初始化所有的游戏桌子
			for(int i = 0; i < this->desk_count_; i++)
			{
				this->desks_.push_back(new ddz_desk(i));
			}
		...
		{
			printf("\nddz_plugin::on_client_leave_desk:%s", client->id());
			//找到这个用户在游戏桌子上的位置索引
		...
		//当游戏桌子开始新一局游戏
		void ddz_plugin::on_game_start(void* desk_, void* null, int verifi_code)
		{
			ddz_desk* desk = (ddz_desk*)desk_;
		...
		//当游戏开始叫牌
		void ddz_plugin::on_game_bid(void* desk_, void* null, int verifi_code)
		{
			ddz_desk* desk = (ddz_desk*)desk_;
		...
		//当前游戏桌子的地主已经决定
		void ddz_plugin::on_game_landlord(void* desk_, void* landlord_, int verifi_code)
		{
			ddz_desk* desk = (ddz_desk*)desk_;
			...
			if(desk->verifi_code != verifi_code) log("397");
		...
		void ddz_plugin::on_game_stop(ddz_desk* desk, game_client* client, bool normal)
		{
			...
			int game_tax = 0;
			desk->write_to_everyone((normal)? "STP 1" : "STP 0");
			...
			if(normal)
			{
				...
				else
				{
					...
					for(int curr_index = 0; curr_index < ddz_desk::DESK_CAPACITY; ++curr_index)
					{
						...
						if(curr_game_info == NULL)log("655");
...
ROBOT_BID:
		...
		void ddz_plugin::on_robot_card(void* arg_desk, void* arg_client, int verify_code)
		{
		...
		{
			//printf("\nddz_plugin::on_client_bye_handle:%s", client->id());
			ddz_game_info* game_info = (ddz_game_info*)client->get_game_info();
