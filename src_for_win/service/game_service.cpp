...
namespace dooqu_server
{
	namespace service
	{
		...
		game_plugin* game_service::create_game(char* game_id, char* title, char* zoneid, int frequence, ...)
		{
			...
			game_plugin* game = new dooqu_server::ddz::ddz_plugin(this, game_id, title, frequence, arguments.size(), game_argv);
		...
		void game_service::on_client_leave(game_client* client, int leave_code)
		{
			...
			printf("\nEND game_service.on_client_leave.{%d}", client->name(), leave_code);
		}
		...
		void game_service::end_auth(const boost::system::error_code& code, void* user_data, std::string& response_string)
		{
